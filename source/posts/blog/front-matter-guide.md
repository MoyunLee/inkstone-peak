...
**详情页只有一套壳子**：案例页与文章页共用同一个外壳——顶图槽（有自托管视频出视频 → 声明 `embed_hero: true` 时出第一条嵌入 → 有图出图 → 都没有不出）· 题头（标题 → 发丝线）· 正文 · 嵌入槽 · 版权槽 · 侧栏（一张信息卡：作者 / 提要 / 发布 · 更新于 · 分类 · 标签；目录在卡外自成一块）全是同一份实现；**提要与元信息只住侧栏这一处**（关掉侧栏的篇目才回落到标题下）；作品只多一项「周期」，并把类型标签显示为 `site.yml` 里 `blog.work_tag` 的「作品」——两型的差别在页面上只剩标签与作品自带的数据。

**两个视图、一份数据**：`/blog` 是中心库，展示**全部**文章（含作品）；`/portfolio` 只是同一份数据里带标记的那个子集。两者由同一次构建产出，不存在两份事实。

内容槽与作品字段（都可缺省，缺省即不渲染那一块）；其中 `links` / `video` / `embeds` **两型通用**——博文同样能挂播放器，`period` / `carousel` 只对作品有意义：

| 字段 | 必填 | 说明 |
|---|---|---|
| `description` | 可选 | 一句话提要：卡片与详情页侧栏（署名之下）都用它（就是「全字段速查」里的那个 `description`，作品不另起字段） |
| `period` | 可选 | 项目周期，出现于案例页元信息行与侧栏行（`2026.09 – 2026.12`）；作品专属 |
| `links` | 可选 | 外链 `[{ label, url }]`（必须合法 http(s)）；两型通用 |
| `video` | 可选 | 站内自托管视频 `{ src, poster, controls, caption }`；两型通用，占顶部槽位 |
| `embeds` | 可选 | 第三方播放器嵌入（仅白名单平台，必须 https）；两型通用，缺省全部留在正文前的「视频」分节（要占顶见下一行） |
| `embed_hero` | 可选 | `true` = 第一条嵌入占顶部槽（16/9 定比、独占版心）；**缺省 `false`** = 嵌入只走正文前的「视频」分节 |
| `carousel` | 可选 | `true` = 上观山顶部纯图轮播；**必须同时有 `cover`** 才进得去；作品专属 |

### 正文中间插视频

正文里**独立成行**写这一行就行（`html: false` 的安全不变量不动——这是自定义语法，不是放行原始 HTML）：

```text
@[B 站 · 《重回1985》12 集全集连播](https://player.bilibili.com/player.html?bvid=BV1mfK767EqC&autoplay=0&high_quality=1)
```

- 方括号里是**标签**（作 iframe 的可访问名，必填）；圆括号里必须是**播放器地址**。三道闸与 front-matter 的 `embeds` 完全同口径（https / B 站页面地址当场教换 / host 白名单），**构建期报错并指出文件**，坏块不会交付。
- **独立成行才生效**（块级规则）；写在句子中间不识别，会原样留在段落里。
- 渲染成与「视频」分节**同一套 markup**（宽度同 `--prose-max`，与正文阅读宽同档），零新增 CSS、零前端 JS，预渲染照旧带播放器。
- 想让它占**顶部槽**（16/9 独占版心）用 front-matter 的 `embed_hero: true`；想让整篇只有一个视频位就只写一处。
...
## 七、置顶序：swiper_index 与 top_group_index

设了 `swiper_index` 的排在最前，其次是设了 `top_group_index` 的，数字越小越靠前；两个都没设就按 `date` 倒序。本文设了 `swiper_index: 1`，所以它排在归档最前面。

**这一份置顶序是全站唯一的排序契约**（实现只有一处：`src/lib/data/content.ts` 的 `compareArticles()`），四处列表吃同一份：

| 列表 | 顺序 | 谁能置顶 |
|---|---|---|
| `/blog` 归档网格 + 首页造境段 | 同一契约（日期方向由 `site.yml` `blog.order` 给） | 全部文章（含作品） |
| 观山列表 + **观山顶部轮播** + 首页观山段 | 同一契约 + `date` 倒序 | 作品 |

> ⚠ **与 Butterfly 的关系**：这两个字段**不是**一套独立的「轮播图列表」，但**确实管观山顶部轮播的顺序**——轮播成员＝作品里声明 `carousel: true` 且有 `cover` 的前 `max_slides` 张，**顺序就是这个置顶序**（例：给某件作品写 `swiper_index: 1`，它立刻排到轮播第一张）。轮播的**张数上限与切换间隔**才在 `site.yml` 的 `portfolio.carousel`（`max_slides` / `interval_ms`）。
...
| `swiper_index` = 首页轮播图 | 本站 = 全站置顶序号：归档网格 + 观山列表 + 观山顶部轮播（见第七节） |
